...
The same parser serves any OpenAI-compatible endpoint. Groq and Ollama both
speak that protocol, so both are configurations of the class below rather than
two implementations of the same thing (TR-084).
"""
...
if TYPE_CHECKING:  # pragma: no cover - type-check-time assertion, never executed
    ...
    def _satisfies_protocol(provider: OpenAICompatibleLLM) -> LLMProvider:
        """Assert at type-check time that the provider implements the protocol.
...
class OpenAICompatibleLLM:
    """Stream assistant tokens and tool calls from an OpenAI-compatible endpoint.

    Groq and Ollama both expose ``/chat/completions`` with the same request and
    SSE shapes, so they differ only in a base URL, a model name, and whether a
    credential is sent. Everything that is genuinely hard here -- reassembling
    tool-call arguments from fragments, distinguishing the three SSE line
    terminators, releasing the connection deterministically when a turn is
    cancelled -- is the same for both and is written once.
    ...
    Args:
        name: Provider name reported to the health probe and used by the error
            mapper to decide which failure code a client sees.
        model: Model identifier.
        base_url: OpenAI-compatible base URL.
        temperature: Sampling temperature.
        max_tokens: Response cap.
        api_key: Credential, when the endpoint wants one. A local endpoint does
            not, and sending an empty bearer token to one is worse than sending
            nothing.
    ...
    async def stream(
        ...
        try:
            async with self._client.stream(
                ...
                await _raise_for_status(response, self._model, self.name)
                ...
                async with aclosing(_iter_sse_payloads(response)) as payloads:
                    async for raw in payloads:
                        for event in _events_from_payload(raw, state, self.name):
                            yield event
        except (httpx.HTTPError, httpx.InvalidURL) as exc:
            raise _wrap_transport_error(exc, self.name) from exc
...
async def _raise_for_status(response: httpx.Response, model: str, provider: str) -> None:
    ...
    Args:
        ...
        provider: Which provider failed. Stamped on the error because the session maps
            it to the code the client sees, and a hosted failure reported as a local
            one would put the wrong message in front of the user.
...
def _wrap_transport_error(exc: httpx.HTTPError | httpx.InvalidURL, provider: str) -> ProviderError:
...
def _events_from_payload(payload: str, state: _StreamState, provider: str) -> list[LLMEvent]:
    ...
    if error is not None:
        raise ProviderError(provider, f"upstream error: {_error_message(error)}")
